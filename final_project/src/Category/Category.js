...
class Category extends React.Component{
	...
	componentDidUpdate(prevProps){
		...
		} else if(!prevProps.writingMessage && this.props.writingMessage){  //若文章提交成功就將輸入框和提醒清空
			this.setState({
				writing: '',
				categoryState: category,
				release: '公開',
	...
	handleWritingSubmit = (e) => {
		e.preventDefault()
		const {writing, categoryState, release} = this.state
		...
		if(writing === ''){  //確認是否輸入內容
		...
		} else if(categoryState === ''){  //確認是否選擇發文類別
			this.setState({
			...
			this.props.writingSubmit(account, writing, categoryState, release)
		}
	}
	...
	handleChangePage = (e) => {
		...
		this.props.history.push('/category/' + category + '/' + page)
		this.props.getCategoryWriting(page, category)
	}
	//前一頁功能
	handlePrevPage = (e) => {
	...
	render(){
		const { nowLoading, loginAccount, loginNickname, loginImage, writingData, writingLike, commentData, newComment, newComment_num, isLoading, pagination, currentPage} = this.props
		const { writing, formAlert, categoryState, release, modifyId, modifyWriting, modifyCategory, modifyRelease, comment, commentDiv } = this.state
			...
			<div id = 'mainDiv' style = {{
			...
			}}>
				{
					...
					<div className = 'writing_input'>
						<Link to={`/mypage/${loginAccount}/1`} className = 'writing_user'>
							{
								!loginImage ?
						...
						<form className = 'writing_form'>
							...
							<div className = 'writing_setup'>
								<select name="categoryState" value = {categoryState} className = {'writing_setup_menu' + (formAlert === 'No category' ? ' alert' : '')} onChange = {this.onInputChange} >
